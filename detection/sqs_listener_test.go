...
func TestS3NotificationReloadSignaller(t *testing.T) {
	t.Run("matching_event_signals_reload", func(t *testing.T) {
		reloadCh := make(chan struct{}, 1)
		responder := S3NotificationPluginReloadResponder("test_plugin", "some_bucket", reloadCh)

		responder(&S3EventNotification{
		...
		case <-time.After(time.Second):
			t.Fatal("expected a reload signal for a matching event")
		}
	})

	t.Run("wrong_bucket_does_not_signal", func(t *testing.T) {
		reloadCh := make(chan struct{}, 1)
		responder := S3NotificationPluginReloadResponder("test_plugin", "some_bucket", reloadCh)
		...
		select {
		case <-reloadCh:
			t.Fatal("a mismatched bucket must not signal a reload")
		case <-time.After(200 * time.Millisecond):
		}
	})

	t.Run("wrong_key_does_not_signal", func(t *testing.T) {
		reloadCh := make(chan struct{}, 1)
		responder := S3NotificationPluginReloadResponder("test_plugin", "some_bucket", reloadCh)
		...
		select {
		case <-reloadCh:
			t.Fatal("a mismatched object key must not signal a reload")
		case <-time.After(200 * time.Millisecond):
		}
	})

	t.Run("wrong_event_name_does_not_signal", func(t *testing.T) {
		reloadCh := make(chan struct{}, 1)
		responder := S3NotificationPluginReloadResponder("test_plugin", "some_bucket", reloadCh)
		...
		select {
		case <-reloadCh:
			t.Fatal("a non ObjectCreated:Put event must not signal a reload")
		case <-time.After(200 * time.Millisecond):
		}
	})

	t.Run("listener_error_does_not_signal", func(t *testing.T) {
		reloadCh := make(chan struct{}, 1)
		responder := S3NotificationPluginReloadResponder("test_plugin", "some_bucket", reloadCh)

		responder(nil, errors.New("something went wrong"))

		select {
		case <-reloadCh:
			t.Fatal("an error must not signal a reload")
		case <-time.After(200 * time.Millisecond):
		}
	})
}
